async function fetchStatus() {
    try {
        ...
        const hazardReason = document.gasync function fetchStatus() {
    try {
        const response = await fetch("/api/status");
        const result = await response.json();
        ...
        const cameraHazardResult = document.getElementById("cameraHazardResult");
        const cameraLastCapture = document.getElementById("cameraLastCapture");
        ...
        if (statusLabel) {
            statusLabel.textContent = data.status || "SAFE";
        }
        ...
        if (cameraStatus) {
            cameraStatus.textContent = data.camera_status || "Idle";
        }
        ...
        if (dashboardStatusTitle) {
            dashboardStatusTitle.textContent = data.status || "SAFE";
        }

        if (dashboardEnvironment) {
            dashboardEnvironment.textContent = data.environment || "Stable walking surface";
        }

        if (hazardReason) {
            hazardReason.textContent = data.hazard_reason || "No immediate hazard detected.";
        }

        if (mobilityMessage) {
            mobilityMessage.textContent = data.mobility_message || "Path appears stable.";
        }

        if (cameraHazardResult) {
            cameraHazardResult.textContent = data.camera_hazard_result || "No camera analysis yet.";
        }

        if (cameraLastCapture) {
            cameraLastCapture.textContent = data.camera_last_capture || "Not yet captured";
        }
    } catch (error) {
...
async function fetchTrustedContact() {
    try {
        ...
        if (savedName) {
            savedName.textContent = contact.name || "Not set";
        }

        if (savedInfo) {
            savedInfo.textContent = contact.contact || "Not set";
        }

        if (savedType) {
            savedType.textContent = contact.contact_type || "unknown";
        }
    ...
    try {
        ...
        if (contactMessage) {
            contactMessage.textContent =
                result.message || (result.success ? "Trusted contact saved." : "Failed to save trusted contact.");
        }
...
async function analyzeCamera() {
    ...
    try {
        ...
        if (cameraHazardResult) {
            cameraHazardResult.textContent = "Analyzing camera view...";
        }
        ...
        if (!result.success) {
            ...
            if (cameraHazardResult) {
                cameraHazardResult.textContent = result.message || "Camera analysis failed.";
            }
            return;
        }

        if (cameraStatus) {
            cameraStatus.textContent = result.camera_status || "Complete";
        }

        if (cameraHazardResult) {
            cameraHazardResult.textContent = result.camera_hazard_result || "No camera analysis yet.";
        }

        if (cameraLastCapture) {
            cameraLastCapture.textContent = result.camera_last_capture || "Not yet captured";
        }

        await fetchStatus();
    } catch (error) {
        console.error("Error analyzing camera:", error);
        if (cameraStatus) {
            cameraStatus.textContent = "Error";
        }
        if (cameraHazardResult) {
            cameraHazardResult.textContent = "Error analyzing camera.";
        }
    }
}
