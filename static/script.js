async function fetchStatus() {
    try {
        ...
        if (cameraHazardResult) cameraHazardResult.textContent = data.camera_hazard_result || "No camera analysis yet.";
        if (cameraLastCapture) cameraLastCapture.textContent = data.camera_last_capture || "Not yet captured";
    } catch (error) {
...
async function analyzeCamera() {
    ...
    try {
        ...
        if (cameraHazardResult) cameraHazardResult.textContent = "Analyzing camera view...";
        ...
        if (!result.success) {
            ...
            if (cameraHazardResult) cameraHazardResult.textContent = result.message || "Camera analysis failed.";
            return;
        }

        if (cameraStatus) cameraStatus.textContent = result.camera_status || "Complete";
        if (cameraHazardResult) cameraHazardResult.textContent = result.camera_hazard_result || "No camera analysis yet.";
        if (cameraLastCapture) cameraLastCapture.textContent = result.camera_last_capture || "Not yet captured";

        await fetchStatus();
    } catch (error) {
        console.error("Error analyzing camera:", error);
        if (cameraStatus) cameraStatus.textContent = "Error";
        if (cameraHazardResult) cameraHazardResult.textContent = "Error analyzing camera.";
    }
}
